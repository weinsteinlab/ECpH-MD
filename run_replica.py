...
lambda_list = pd.read_csv(('lambda_list-%s.csv' % (str(pH))), index_col=0)
...
if restart == 'OFF' and iteration == 0:
    ...
    dcdReporter = DCDReporter('%s-%s-%s.dcd ' % (str(output_name), str(pH), str(iteration)), dcdout_freq)
    dataReporter = StateDataReporter('%s-%s.log' % (str(output_name), str(pH))), 1000, totalSteps=nsteps, step=True, time=True, speed=True, progress=True, elapsedTime=True, remainingTime=True, potentialEnergy=True, kineticEnergy=True, totalEnergy=True, temperature=True, volume=True, density=True, separator=',')
    ...
    simulation.saveState('%s-%s-state.xml' % (str(output_name),str(pH)))
    ...
    energy.to_csv('%s-%s-energy.csv' % (str(output_name), str(pH)))
    ...
    energy_min.to_csv('%s-%s-energy-min.csv' % (str(output_name), str(pH)))
    print('FINISH')
else:
    print('Iteration ', iteration, ' pH ', pH)
    simulation.loadState('%s-%s-state.xml' % (str(output_name), str(pH)))
    ...
    state = simulation.context.getState(getPositions=False, getVelocities=False, getForces=False, getEnergy=True,
      ...
      getParameterDerivatives=False)
    energy = state.getKineticEnergy()._value + state.getPotentialEnergy()._value
    energy_input = pd.read_csv(('%s-%s-energy.csv' % (str(output_name), str(pH))), index_col=0)
    ...
    output.to_csv('%s-%s-energy.csv' % (str(output_name), str(pH)))
    simulation.minimizeEnergy(maxIterations=1000)
    state_min = simulation.context.getState(getPositions=False, getVelocities=False, getForces=False, getEnergy=True,
      getParameters=False,
      getParameterDerivatives=False)
    energy_min = state_min.getKineticEnergy()._value + state_min.getPotentialEnergy()._value
    energy_min_input = pd.read_csv(('%s-%s-energy-min.csv' % (str(output_name), str(pH))), index_col=0)
    ...
    output.to_csv('%s-%s-energy-min.csv' % (str(output_name), str(pH)))
    print('FINISH')
