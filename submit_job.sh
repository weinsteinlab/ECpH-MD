...
for (( subjob=0; subjob<$number_of_subjobs; subjob++ )); do
    ...
    if [ $first_subjob -eq 0 ]; then
        ...
        jobSchedulerOutput="$(sbatch --depend=afterok:${job_scheduler_number} -J ${jobName} -N ${numberOfNodes} -p $partitionName --mem=80G --gres=gpu:${number_of_GPUs_per_node} -t 0-02:00:00 ./submit_Exchange-min-replica.sh ${number_of_replicas})"
    fi
