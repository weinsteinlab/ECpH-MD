...
for (( subjob=0; subjob<$number_of_subjobs; subjob++ )); do
    ...
    if [ $first_subjob -eq 0 ]; then
        jobSchedulerOutput="$(sbatch -o "./submission_logs/slurm_%A.out" -J ${jobName} -N ${numberOfNodes} -p $partitionName --mem=80G --gres=gpu:${number_of_GPUs_per_node} -t 0-02:00:00 ./submit-replicas.sh ${number_of_replicas} 0)"

    else
        jobSchedulerOutput="$(sbatch --depend=afterok:${job_scheduler_number} -o "./submission_logs/slurm_%A.out"  -J ${jobName} -N ${numberOfNodes} -p $partitionName --mem=80G --gres=gpu:${number_of_GPUs_per_node} -t 0-02:00:00 ./submit-replicas.sh ${number_of_replicas})"
    fi
